...
roc.add_curve('gradient trees (training)',
    train_labels, train_predictions.numpy(),
    plot_args = dict(color='C0', linestyle='--'))
roc.add_curve('gradient trees (validation)',
    val_labels, val_predictions.numpy(),
    plot_args = dict(color='C0', linestyle='-'))
WP = roc.get_WP('gradient trees (validation)', threshold=0.5)
roc.draw_WP(WP)
